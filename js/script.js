$(document).ready(function(){
    ...
    $('.allproduct, .placeopen, .specialopen, .newsopen, .ideaopen, .selfplanopen').click(function(event) {
    ...
    $('.allproduct').click(function() {
        $('.all_menu').show(); // 애니메이션 없이 나타나게
    });
    $(".cls_allmenu").click(function(){
        $(".all_menu").hide(); 
    });
    $('.placeopen').click(function() {
        $('.place_shop').show(); // 애니메이션 없이 나타나게
    });
    $(".cls_placemenu").click(function(){
        $(".place_shop").hide(); 
    });
    $('.specialopen').click(function() {
        $('.special').show(); // 애니메이션 없이 나타나게
    });
    $(".cls_placemenu").click(function(){
        $(".special").hide(); 
    });
    $('.newsopen').click(function() {
        $('.news').show(); // 애니메이션 없이 나타나게
    });
    $(".cls_placemenu").click(function(){
        $(".news").hide(); 
    });
    $('.ideaopen').click(function() {
        $('.idea').show(); // 애니메이션 없이 나타나게
    });
    $(".cls_placemenu").click(function(){
        $(".idea").hide(); 
    });
    $('.selfplanopen').click(function() {
        $('.selfplan').show(); // 애니메이션 없이 나타나게
    });
    $(".cls_placemenu").click(function(){
        $(".selfplan").hide(); 
    });
    $('.cls_menu_all').click(function() {
        $('.menu_click, .all_menu, .place_shop, .special, .news, .idea, .selfplan').hide(); // 모두 숨기기
    });
    // // 메뉴클릭
    ...
    // });
    // $('.cls_allmenu').click(function() {
    //     $('.menu_click').css({ right: '0' }).show(); // 애니메이션 없이 나타나게
    // });
    // // $(document).click(function(event) {
    ...
    // // });
    // $(document).click(function(event) {
    //     // 메뉴가 열려 있고 클릭한 요소가 '.menu_click' 또는 '.header_menu'가 아닌 경우
    //     if ($(".menu_click").is(":visible") && !$(event.target).closest('.menu_click').length && !$(event.target).closest('.header_menu').length) {
    //         $(".menu_click").hide(); // 메뉴를 숨김
    //     }
    // });
    ...
    window.addEventListener('scroll', () => {
        const arts = document.querySelectorAll('.box > div');
        ...
        arts.forEach(art => {
            ...
            // 요소가 뷰포트에 들어올 때
            if (artTop < windowHeight - 100) {
                art.classList.add('visible'); // visible 클래스 추가
            } else {
                art.classList.remove('visible'); // visible 클래스 제거
            }
        });
    });
});